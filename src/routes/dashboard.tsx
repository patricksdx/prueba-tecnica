...
function Dashboard() {
	...
	const maxSales = Math.max(
		...data.monthlySales.map((month) => month.sales),
		1,
	);
	...
	return (
		<div className="sales-app">
			<header className="sales-topbar">
				<a className="sales-brand" href="/dashboard">
			...
			<main className="sales-content">
				<div className="sales-heading">
					<div>
						<p className="sales-eyebrow">REPORTE COMERCIAL · 2026</p>
						<h1 className="text-3xl font-semibold tracking-tight">Ventas</h1>
						<p className="text-sm text-muted-foreground">
							Pedidos, productos y desempeño de vendedores.
						</p>
					</div>
					<span className="source-badge">
						<Package size={15} /> Datos de pedidos
					</span>
				</div>
				{error && (
				...
				<section
					className="sales-panel flex items-center gap-4"
					aria-label="Total de ventas"
				>
				...
				<div className="sales-grid">
					<section className="sales-panel">
						<h2 className="font-semibold">Evolución de ventas mes a mes</h2>
						<p className="mb-5 text-xs text-muted-foreground">
							Importes de pedidos completados · PEN
						</p>
						<div
							className="monthly-chart"
							role="img"
							aria-label="Gráfica de ventas mensuales"
						>
							{data.monthlySales.map((month) => (
								<div
									className="month-column"
									key={month.month}
									title={`${month.month}: ${currency(month.sales)}`}
								>
									<span className="month-value">
										{month.sales ? currency(month.sales) : "—"}
									</span>
									<div className="month-bar-track">
										<div
											className="month-bar"
											style={{
												height: `${month.sales ? Math.max(4, (month.sales / maxSales) * 100) : 0}%`,
											}}
										/>
									</div>
							...
							{data.products.map((product, index) => (
								...
								>
									...
									<div className="min-w-0 flex-1">
										<p className="truncate font-medium" title={product.product}>
				...
				<section className="sales-panel">
					<h2 className="font-semibold">Ventas por vendedor</h2>
					<p className="mb-4 text-xs text-muted-foreground">
						Ventas en pedidos completados
					</p>
					<DataTable
				...
				<section className="sales-panel">
					...
					<p className="mb-4 text-xs text-muted-foreground">
						Selecciona el número de pedido para ver su detalle ·{" "}
				...
				{data.user.role === "admin" && (
					<section className="sales-panel">
						...
						<p className="mb-4 text-xs text-muted-foreground">
							Las cuentas nuevas quedan en espera hasta que se les asigne un
							rol.
						</p>
						<DataTable
							columns={userColumns(
								data.user.clerkId,
								saving,
								(id, role) => void updateRole(id, role),
							)}
